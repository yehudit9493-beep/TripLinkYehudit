...
//       next: (data) => {
//         this.allGuides = data;
//         this.ratingService.loadAverages('guide', data.map(g => g.id));
//         this.applyFilters();
...
@Component({
  ...
  styleUrl: './guide.scss',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush // ✅ חשוב!
})
export class Guide implements OnInit, OnDestroy {

  private _liveAnnouncer = inject(LiveAnnouncer);
  private cdr = inject(ChangeDetectorRef); // ✅ להודעת שינויים
  ...
  ngAfterViewInit() {
    ...
    this.dataSource.sortingDataAccessor = (item: Guides, property: string) => {
      switch (property) {
        case 'trainingRegions':
          return item.trainingRegionsId?.toString() || '';
        case 'yearsOfExperience':
          return item.yearsOfExperience;
        case 'rating':
          return this.getAverageRating(item.id);
        default:
